...
bot.on('message', async message => {
    ...
    if(message.content.toLowerCase().startsWith("?play"))
    {
        ...
        let url = args[1];
        let VoiceChannel = message.guild.channels.cache.find(channel => channel.id === '708065455247458304');
    ...
    else if(message.content.toLowerCase().startsWith("?queue"))
    {
        let args = message.content.split(" ");
        let url = args[1];
        let VoiceChannel = message.guild.channels.find(channel => channel.id === '708065455247458304');
